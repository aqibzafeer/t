...
export default function Home() {
  return (
    <>
      <div className="relative w-full h-screen flex flex-col justify-center items-center text-center px-6 bg-cover bg-center" style={{ backgroundImage: `url(${HERO.background})` }}>
        <div className="relative z-10 max-w-2xl text-white">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-6 drop-shadow-lg"> {HERO.title} </h1>
          <p className="text-lg md:text-xl text-gray-200 my-5 drop-shadow-md"> {HERO.description} </p>
      ...
      {ABOUT.map((section, i) => (
        <section key={i}  className="w-full bg-gray-50 py-16 px-6 md:px-16" >
          <div className={`max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center ${section.reverse ? "md:flex-row-reverse" : "" }`}>{!section.reverse && (
            <div className="flex justify-center">  
            <img src={section.image} alt={section.title} className="rounded-2xl shadow-lg w-full object-cover" /> 
            </div>)}
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-red-800 mb-4 flex items-center gap-2"> {section.icon} {section.title} </h2>
              <p className="text-gray-700 text-lg leading-relaxed">{section.text}</p>
            </div>

            {section.reverse && (
              <div className="flex justify-center"><img src={section.image} alt={section.title} className="rounded-2xl shadow-lg w-full object-cover" /> </div>
            )}
          </div>
        </section>
      ))}
      ...
      <div className="bg-white py-16 px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {VALUES.map((item, i) => (
            <div key={i} className="bg-red-50 hover:bg-red-100 transition-colors duration-300 p-8 rounded-2xl shadow-md text-center">
              {item.icon}
      ...
      <section className="w-full py-16 px-6 md:px-16 bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {CATG.map((product, i) => (
            <div key={i} className="bg-gray-50 hover:bg-red-50 transition-colors duration-300 rounded-xl shadow-md hover:shadow-xl p-6 flex flex-col items-center text-center" >
              {product.icon}
              <h3 className="text-xl font-semibold text-gray-800 mt-4 mb-2"> {product.name}</h3>
              <p className="text-gray-600 text-sm"> Experience the authentic taste and aroma of {product.name}. </p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="relative bg-red-900 py-16 px-6 md:px-12 text-center text-white">
        <h2 className="text-3xl md:text-4xl font-bold mb-4 flex justify-center items-center gap-2"> <FaHeart /> {JOIN.title} </h2>
        <p className="max-w-2xl mx-auto mb-8 text-gray-200">{JOIN.description}</p>
        <Link href={JOIN.buttonHref} className="inline-block bg-white text-red-900 font-semibold px-6 py-3 rounded-full shadow hover:bg-red-100 transition duration-300">  {JOIN.buttonText}</Link>
      </div>

{/* ✅ HOMEPAGE PRODUCTS PREVIEW SECTION */}
<section className="w-full py-16 px-6 md:px-16 bg-white">
  <div className="max-w-6xl mx-auto text-center mb-10">
    <h2 className="text-3xl md:text-4xl font-bold text-red-800 mb-4 flex justify-center items-center gap-2">
    ...
    <p className="text-gray-600 max-w-2xl mx-auto">
      Explore our handpicked premium teas crafted to rejuvenate your body and calm your mind.
    </p>
  </div>
